...
| F-APP-002 | Chat list | The chat list renders empty and populated states, explains the observable DNS signal path, exposes a primary new-chat action, and routes into a thread. | AXe selectors: `chat-list`, `chat-list-new-chat`; ChatContext, storage, web-runtime, and UI contract Jest tests. |
...
| F-USER-001 | Profile and data management | The profile route shows user identity, conversation/message statistics, settings navigation, and clear-all data confirmation. | AXe selectors: `profile-screen`, `profile-settings-link`, `profile-clear-all-data`; route parsing and chat context tests. |
