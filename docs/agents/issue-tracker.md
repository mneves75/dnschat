...
- **Create an issue** (when authorized): `gh issue create --title "..." --body-file <FILE>`. Write and inspect the body file before sending it.
- **Read an issue**: `gh issue view <number> --json number,title,body,comments,labels`.
- **List issues**: `gh issue list --state open --json number,title,body,labels,comments --jq '[.[] | {number, title, body, labels: [.labels[].name], comments: [.comments[].body]}]'` with appropriate `--label` and `--state` filters.
- **Comment on an issue** (when authorized): `gh issue comment <number> --body-file <FILE>`
...
- **Read a PR**: `gh pr view <number> --json number,title,body,comments,labels` and `gh pr diff <number>` for the diff.
...
Create a GitHub issue only when external writing is authorized by the user.

## When a skill says "fetch the relevant ticket"

Run `gh issue view <number> --json number,title,body,comments,labels`.
