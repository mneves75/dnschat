...
const GlassChatItem: React.FC<ChatItemProps> = ({ 
  ...
  console.log('🔍 GlassChatItem rendered for chat:', chat.title);
  console.log('🔍 onShare prop exists:', !!onShare);
  ...
  const handleLongPress = React.useCallback(() => {
    console.log('🚨 EMERGENCY DEBUG: handleLongPress called for chat:', chat.title);
    console.log('🚨 EMERGENCY DEBUG: onShare available:', !!onShare);
    
    // EMERGENCY TEST: Replace action sheet with simple Alert
    Alert.alert(
      '🚨 LONG PRESS WORKS!',
      `Chat: ${chat.title}\nonShare: ${!!onShare ? 'YES' : 'NO'}`,
      [
        { text: 'Cancel', style: 'cancel' },
        { text: 'Test Share', onPress: () => {
          console.log('🚨 EMERGENCY: Testing handleShareChat directly');
          if (onShare) {
            onShare();
          }
        }}
    ...
    // Keep original action sheet call for comparison
    // console.log('🔍 About to show action sheet');
    // actionSheet.show();
  }, [chat.title, onShare]);
        ...
        <View style={styles.chatInfo}>
          ...
          <View style={styles.chatMeta}>
            ...
            <View style={styles.chatBadges}>
              {messageCount > 0 && (
                <View style={[styles.messageBadge, { backgroundColor: 'rgba(0, 122, 255, 0.15)', borderRadius: 12 }]}>
      ...
      <GlassActionSheet
        ...
        actions={[
          ...
          ...(onShare ? [{
            title: 'Share Chat',
            onPress: () => {
              console.log('🔍 Share Chat action pressed from action sheet');
              onShare();
            },
            icon: <Text>📤</Text>,
          }] : []),
...
export function GlassChatList() {
  ...
  const formatChatForSharing = React.useCallback((chat: Chat): string => {
    console.log('🔍 formatChatForSharing called for chat:', chat.title);
    ...
    const formatMessage = (message: any) => {
      ...
      const timestamp = formatDate(new Date(message.timestamp));
      const content = message.content.trim();
      
      return `[${timestamp}] ${role}: ${content}`;
    };

    const header = `📱 DNS Chat Conversation\n` +
                  `💬 ${chat.title}\n` +
    ...
    const footer = `\n\n${'─'.repeat(50)}\n` +
                  `\n🔗 Generated by DNS Chat\n` +
                  `🌐 Chat over DNS - github.com/mneves75/dnschat`;

    const result = header + messages + footer;
    console.log('🔍 Final formatted result length:', result.length);
    return result;
  }, []);

  const handleShareChat = React.useCallback(async (chat: Chat) => {
    console.log('🔍 handleShareChat called with chat:', chat.title);
    ...
    if (!Share || typeof Share.share !== 'function') {
      ...
      Alert.alert('Share Not Available', 'Sharing is not supported on this platform.');
      return;
    }
    
    try {
      console.log('🔍 About to format chat for sharing...');
      const shareContent = formatChatForSharing(chat);
      console.log('🔍 Share content length:', shareContent.length);
      console.log('🔍 Share content preview:', shareContent.substring(0, 200) + '...');
