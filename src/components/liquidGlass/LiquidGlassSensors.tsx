...
interface SensorCallbacks {
  ...
  onPerformanceMode?: (mode: 'quality' | 'balanced' | 'performance' | 'battery') => void;
...
class LiquidGlassSensorManager {
  ...
  async startMonitoring(): Promise<void> {
    ...
    try {
      ...
      this.callbacks.onError?.(error instanceof Error ? error.message : 'Sensor initialization failed');
  ...
  private async updateAllSensors(): Promise<void> {
    ...
    try {
      ...
      const [ambientLight, motionState, thermalState, proximityNear, batteryState] = await Promise.all([
        this.config.enableAmbientLightAdaptation ? SensorModule.getCurrentAmbientLight() : Promise.resolve(1000),
        this.config.enableMotionAdaptation ? SensorModule.getCurrentMotionState() : Promise.resolve({
          isStationary: true,
          acceleration: { x: 0, y: 0, z: 0 },
          rotation: { x: 0, y: 0, z: 0 },
          orientation: 'portrait' as const,
        }),
        this.config.enableThermalAdaptation ? SensorModule.getThermalState() : Promise.resolve('nominal' as const),
        this.config.enableProximityAdaptation ? SensorModule.isProximityNear() : Promise.resolve(false),
        this.config.enableBatteryOptimization ? SensorModule.getBatteryState() : Promise.resolve({
          level: 1,
  ...
  private processAdaptations(trigger: string): void {
    ...
    this.callbacks.onAdaptationChange?.(adaptedIntensity, `${trigger}_adaptation`);
  ...
  private calculateAdaptiveIntensity(): GlassIntensity {
    ...
    if (this.config.enableMotionAdaptation) {
      if (this.currentData.motionState === 'shaking' || this.currentData.motionState === 'moving') {
    ...
    if (this.config.enableThermalAdaptation) {
      if (this.currentData.thermalState === 'serious' || this.currentData.thermalState === 'critical') {
    ...
    if (this.config.enableProximityAdaptation && this.currentData.proximityNear) {
      baseIntensity = this.reduceIntensity(baseIntensity);
    }
    
    // Battery optimization
    if (this.config.enableBatteryOptimization) {
      if (this.currentData.lowPowerMode || this.currentData.batteryLevel < 0.2) {
  ...
  private calculatePerformanceMode(): 'quality' | 'balanced' | 'performance' | 'battery' {
    ...
    if (this.currentData.batteryLevel > 0.7 && this.currentData.thermalState === 'nominal') {
      return 'quality';
    }
  ...
  private reduceIntensity(intensity: GlassIntensity): GlassIntensity {
    const intensityLevels: GlassIntensity[] = ['ultraThin', 'thin', 'regular', 'thick', 'ultraThick'];
...
export const useLiquidGlassSensorAdaptation = (
  ...
  const [adaptiveIntensity, setAdaptiveIntensity] = useState<GlassIntensity>('regular');
  const [performanceMode, setPerformanceMode] = useState<'quality' | 'balanced' | 'performance' | 'battery'>('balanced');
  ...
  const startMonitoring = useCallback(async () => {
    ...
    try {
      managerRef.current = new LiquidGlassSensorManager(finalConfig, enhancedCallbacks);
      await managerRef.current.startMonitoring();
      setIsMonitoring(true);
      setError(null);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to start sensor monitoring';
      setError(errorMessage);
  ...
  const stopMonitoring = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to stop sensor monitoring';
      setError(errorMessage);
    }
  }, []);
...
export const useAmbientLightAdaptation = () => {
  const { sensorData, adaptiveIntensity, startMonitoring, stopMonitoring, isAvailable } = 
...
export const useBatteryOptimization = () => {
  const { sensorData, performanceMode, startMonitoring, stopMonitoring, isAvailable } = 
    useLiquidGlassSensorAdaptation({
