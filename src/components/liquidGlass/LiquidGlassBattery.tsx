...
const DEFAULT_CONFIG: BatteryConfig = {
  ...
  aggressiveOptimizationThreshold: 0.2, // 20% battery
...
class LiquidGlassBatteryManager {
  ...
  private startBatteryMonitoring(): void {
    ...
    if (Platform.OS === "ios") {
      DeviceEventEmitter.addListener(
        "batteryStateChanged",
        this.handleBatteryStateChange.bind(this),
      );
    }
  }
  ...
  private handleBatteryStateChange(event: any): void {
    ...
    if (
      newLevel < 0.15 &&
      previousLevel >= 0.15 &&
      this.callbacks.onBatteryWarning
    ) {
  ...
  private async updateThermalState(): Promise<void> {
    try {
      if (Platform.OS === "ios" && NativeModules.ThermalManager) {
        const thermalInfo =
          await NativeModules.ThermalManager.getThermalState();
  ...
  private async switchPowerProfile(newProfile: PowerProfile): Promise<void> {
    ...
    this.logOptimization(
      `power_profile_${newProfile.name}`,
      `Battery: ${(this.currentBatteryState.level * 100).toFixed(0)}%, Thermal: ${this.currentThermalState.level}`,
    );
  ...
  private async applyOptimizations(optimizations: string[]): Promise<void> {
    const newOptimizations = optimizations.filter(
      (opt) => !this.activeOptimizations.includes(opt),
    );
    const removedOptimizations = this.activeOptimizations.filter(
      (opt) => !optimizations.includes(opt),
    );
   ...
   */
  private async removeSpecificOptimization(
    optimization: string,
  ): Promise<void> {
    switch (optimization) {
      case "background_suspension":
  ...
  private shouldApplyThermalOptimization(): boolean {
    return (
      this.currentThermalState.level !== "nominal" &&
  ...
  private shouldReduceFrameRate(): boolean {
    return (
      this.currentBatteryState.level < 0.3 ||
  ...
  private shouldDisableAnimations(): boolean {
    return (
      this.currentBatteryState.level < 0.15 ||
      this.currentThermalState.throttleLevel >= 2 ||
      this.isInBackground
    );
  }
  ...
  private updateUsagePatterns(): void {
    ...
    const batteryDrain =
      this.sessionStartBattery - this.currentBatteryState.level;
    ...
    this.usagePattern.typicalBatteryAtStart =
      this.usagePattern.typicalBatteryAtStart * 0.9 +
      this.sessionStartBattery * 0.1;
  ...
  private async saveUsagePatterns(): Promise<void> {
    try {
      await AsyncStorage.setItem(
        "@liquid_glass_usage_patterns",
        JSON.stringify(this.usagePattern),
      );
    } catch (error) {
  ...
  private getProfileChangeReason(): string {
    ...
    if (level < this.config.aggressiveOptimizationThreshold)
      return "Low battery";
...
export const useLiquidGlassBattery = (config?: Partial<BatteryConfig>) => {
  ...
  const [batteryState, setBatteryState] = React.useState(
    manager.getBatteryState(),
  );
  const [powerProfile, setPowerProfile] = React.useState(
    manager.getCurrentPowerProfile(),
  );
  const [activeOptimizations, setActiveOptimizations] = React.useState(
    manager.getActiveOptimizations(),
  );
  ...
  const forcePowerProfile = React.useCallback(
    (profileName: keyof typeof DEFAULT_POWER_PROFILES) => {
