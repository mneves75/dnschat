...
export interface LiquidGlassProps {
  ...
  onGlassApplied?: (platform: string, capabilities: LiquidGlassCapabilities) => void;
}
...
const NativeGlassView: React.FC<LiquidGlassProps> = ({
  ...
  const [NativeComponent, setNativeComponent] = React.useState<React.ComponentType<any> | null>(null);
...
export const LiquidGlassView: React.FC<LiquidGlassProps> = (props) => {
  ...
  const [fallbackStrategy, setFallbackStrategy] = React.useState<FallbackStrategy | null>(null);
  const [isLoading, setIsLoading] = React.useState(true);
  const fadeAnim = useRef(new Animated.Value(0)).current;

  // Determine the best fallback strategy
  const determineFallbackStrategy = useCallback(async (): Promise<FallbackStrategy> => {
    try {
      ...
      } else if (capabilities.platform === 'ios' && capabilities.apiLevel >= 170) {
      ...
      console.warn('LiquidGlass: Strategy determination failed, using plain fallback', error);
      return {
        platform: 'plain',
        component: PlainView,
        capabilities: null,
  ...
  const getComponentForPlatform = (platform: string): React.ComponentType<any> => {
    switch (platform) {
      case 'native': return NativeGlassView;
      case 'blur': return EnhancedBlurView;
      case 'material': return MaterialSurfaceView;
      case 'css': return CSSGlassView;
      default: return PlainView;
    }
  };
...
export const useLiquidGlassCapabilities = () => {
  const [capabilities, setCapabilities] = React.useState<LiquidGlassCapabilities | null>(null);
  const [isLoading, setIsLoading] = React.useState(true);
 ...
 */
export const useAdaptiveGlassIntensity = (baseIntensity: GlassIntensity = 'regular') => {
