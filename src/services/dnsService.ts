...
/**
 * Sanitize a user message into a single DNS label (RFC 1035 mindful):
 * - Validate first
 * - Remove non-printables
 * - Replace control DNS characters with '_'
...
export class DNSService {
    ...
    for (let attempt = 0; attempt < this.MAX_RETRIES; attempt++) {
      try {
        ...
        for (const method of methodOrder) {
          try {
            const result = await this.tryMethod(
              method,
              sanitizedMessage,
              targetServer,
            );
      ...
      const onError = (e: any) => {
        ...
        if (e === undefined || e === null) {
          ...
          if (
            ...
            reject(
              new Error(
                `UDP permission denied - network restrictions detected: ${e.message}`,
              ),
            );
          } else if (
            ...
            reject(
              new Error(
                `UDP network unreachable - connectivity issue: ${e.message}`,
              ),
            );
          } else {
            reject(e);
          ...
          const errorMsg =
            e.message || e.error || e.description || "Unknown UDP socket error";
        ...
        socket.once("message", (response: Buffer, rinfo: any) => {
          try {
            ...
            if ((decoded as any).rcode !== "NOERROR") {
              throw new Error(
                `DNS query failed with rcode: ${(decoded as any).rcode}`,
              );
            }
  ...
  private static async performDNSOverTCP(
    ...
    return new Promise((resolve, reject) => {
      ...
      const onError = (e: any) => {
        ...
        if (e === undefined || e === null) {
          ...
          if (
            ...
            reject(
              new Error(
                `TCP connection refused - DNS server may be blocking TCP port 53: ${e}`,
              ),
            );
          } else if (
          ...
          if (
            ...
            reject(
              new Error(
                `TCP connection timeout - network may be blocking TCP DNS: ${e.message}`,
              ),
            );
          } else if (
            errorMsg.includes("network") ||
            ...
            reject(
              new Error(
                `TCP network unreachable - connectivity issue: ${e.message}`,
              ),
            );
          } else {
            reject(e);
          ...
          // Extract meaningful error information from object
          const errorMsg =
            e.message || e.error || e.description || "Unknown TCP socket error";
          const errorCode = e.code || e.errno || "NO_CODE";
      ...
      try {
        ...
        if (!queryBuffer) {
          throw new Error(
            "DNS packet encoding failed - queryBuffer is null/undefined",
          );
        }
        ...
        socket.on("data", (data: Buffer) => {
          ...
          if (expectedLength > 0 && responseBuffer.length >= expectedLength) {
            try {
              const decoded = dns.decode(
                responseBuffer.slice(0, expectedLength),
              );
  ...
  private static async performDNSOverHTTPS(
    message: string,
    dnsServer: string,
  ): Promise<string[]> {
      ...
      case "never-https":
        const neverHttpsMethods: (
          ...
          Platform.OS === "web"
            ? ["native", "udp", "tcp"]
            : ["native", "udp", "tcp"];
        return enableMockDNS
          ? [...neverHttpsMethods, "mock"]
          : neverHttpsMethods;

      case "prefer-https":
        const preferHttpsMethods: (
          | "native"
          | "udp"
          | "tcp"
          | "https"
          | "mock"
        )[] = ["https", "native", "udp", "tcp"];
        return enableMockDNS
          ? [...preferHttpsMethods, "mock"]
          : preferHttpsMethods;

      case "native-first":
        // New default: Native DNS always first, regardless of other preferences
        const nativeFirstMethods: (
          | "native"
          | "udp"
          | "tcp"
          | "https"
          ...
          Platform.OS === "web"
            ? ["https", "native"]
            : ["native", "udp", "tcp", "https"];
        return enableMockDNS
          ? [...nativeFirstMethods, "mock"]
          : nativeFirstMethods;
          ...
          const result = await this.handleBackgroundSuspension(async () => {
            ...
            if (capabilities.available && capabilities.supportsCustomServer) {
              ...
              try {
                ...
                if (!records) {
                  throw new Error(
                    "Native DNS query returned null/undefined records",
                  );
                }

                if (!Array.isArray(records)) {
                  console.log(
                    "⚠️ NATIVE: Records is not an array, converting...",
                  );
                  const arrayRecords = Array.isArray(records)
                    ? records
                    : [String(records)];
  ...
  static async queryViaCustomService(
    message: string,
    dnsServer?: string,
  ): Promise<string> {
    const sanitizedMessage = this.sanitizeMessage(message);
  ...
  static async testTransport(
    ...
    console.log(
      `🧪 Starting forced transport test: ${transport.toUpperCase()}`,
    );
    ...
    try {
      console.log(
        `🔧 Testing ${transport.toUpperCase()} transport to ${targetServer}`,
      );
      ...
      switch (transport) {
        case "native":
          console.log(
            "🧪 NATIVE TEST: Starting forced native DNS transport test",
          );

          const result = await this.handleBackgroundSuspension(async () => {
            console.log(
              "🔧 NATIVE TEST: Checking capabilities for forced test...",
            );
            ...
            if (capabilities.available && capabilities.supportsCustomServer) {
              console.log(
                "✅ NATIVE TEST: Native DNS available for forced test",
              );
            ...
            console.log(
              "❌ NATIVE TEST: Native DNS not available for forced test",
            );
            throw new Error(
      ...
      console.log(
        `✅ ${transport.toUpperCase()} transport test successful: ${response}`,
      );
      return response;
    } catch (error: any) {
