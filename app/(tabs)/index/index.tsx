...
export default function ChatListScreen() {
  ...
  const handleNewChat = React.useCallback(async () => {
    ...
    try {
      ...
      // /[id] maps to app/(tabs)/index/[id].tsx
      console.log('[NAV DEBUG] Calling router.push with path:', `/${newChat.id}`);
  ...
  const handleChatPress = React.useCallback(
    (chat: any) => {
      ...
      setCurrentChat(chat);
      console.log('[NAV DEBUG] Calling router.push with path:', `/${chat.id}`);
      router.push(`/${chat.id}`);
      console.log('[NAV DEBUG] router.push called successfully');
    },
