...
[![Version](https://img.shields.io/badge/version-4.0.26-blue.svg)](.)
...
Last architecture/dependency verification: `2026-06-05`.
...
Last uploaded/processed TestFlight build: version `4.0.26` build `60`
(`VALID` on `2026-06-05`, distributed through the internal all-builds tester
track). App Store Connect has no App Store version record for `4.0.26` yet, so
App Store submission validation is not applicable for this TestFlight-only
staging build.

- `npx react-doctor@latest --project chat-dns` reports `100 / 100` on
  `2026-06-05`.
- `bun run verify:all` passes on `2026-06-05`.
- `bun run verify:expo-doctor` reports `20/20 checks passed, no issues` on
  `2026-06-03` after the Expo SDK patch alignment (expo `56.0.8`, expo-router
  `56.2.8`, expo-linking `56.0.13`, expo-build-properties `56.0.16`, @expo/ui
  `56.0.15`).
- Native DNS module tests pass on `2026-06-05` (`7` suites passed, `1` skipped;
  `57` tests passed, `13` skipped).
- AXe E2E baseline: 10 feature groups passed in one owned release-simulator
  run on `2026-06-05`.
- Jest baseline on `2026-06-05`: `114` suites passed, `1` skipped; `919` tests
...
- `gitleaks detect` on `2026-06-05` reports `no leaks found` across `372`
  scanned commits.
- `bun audit` on `2026-06-05` reports `No vulnerabilities found`.
...
- Physical-device Release build, install, installed metadata check, and launch
  ...
  install for `4.0.26` build `60` is blocked by local Xcode Development
  provisioning state (`No Accounts` and no matching development profile);
  TestFlight is the verified staging path for this build.
- Last signed TestFlight build: `4.0.26` build `60`, archived with Apple
  Distribution signing, exported, uploaded, and processed to `VALID` in App Store
  Connect on `2026-06-05`; `asc validate testflight` reports `0` errors and
  `0` warnings. Internal App Store Connect IDs and tester group names are
  intentionally omitted from public docs.
- Historical `asc validate testflight` evidence is superseded by build `60`
  validation. App Store submission validation for `4.0.26` is not applicable
  until a matching App Store version record exists.
