...
class ChangelogGenerator {
  ...
  async generate() {
    try {
      ...
      if (this.options.verbose) {
        console.log(
          `📊 Analysis range: ${this.options.since}..${this.options.until}`,
        );
  ...
  analyzeCommit(commit) {
    try {
      ...
      const files = execSync(filesCmd, { encoding: "utf8" })
        .trim()
        .split("\n")
        .filter((f) => f);
  ...
  categorizeCommit(commit, analysis) {
    ...
    if (
      files.some(
        (f) =>
          f.includes("src/") || f.includes("ios/") || f.includes("android/"),
      )
    ) {
  ...
  generateAttribution(commit) {
    ...
    const username =
      this.getGitHubUsername(commit.email) ||
      commit.author.replace(/\s+/g, "").toLowerCase();
  ...
  getFirstTimeContributors(contributors) {
    ...
    return contributors.filter(
      (contributor) => !previousContributors.includes(contributor.email),
    );
  }
  ...
  generateChangelogEntry(changes, firstTimeContributors) {
    ...
    if (firstTimeContributors.length > 0) {
      ...
      firstTimeContributors.forEach((contributor) => {
        const username =
          this.getGitHubUsername(contributor.email) ||
          contributor.name.replace(/\s+/g, "").toLowerCase();
  ...
  isBotContributor(contributor) {
    ...
    return this.botPatterns.some(
      (pattern) => pattern.test(email) || pattern.test(name),
    );
  }
  ...
  getAffectedAreas(files) {
    ...
    files.forEach((file) => {
      ...
      if (file.includes("doc") || file.endsWith(".md"))
