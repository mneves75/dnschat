...
function getNextBuildNumber() {
  try {
    ...
    if (!buildMatch) {
      console.warn('⚠️  Could not find CURRENT_PROJECT_VERSION, defaulting to 1');
      return 1;
    }
    
    return parseInt(buildMatch[1]) + 1;
  } catch (error) {
    console.warn('⚠️  Could not read iOS project file, defaulting to build number 1');
    return 1;
  }
}
...
function updatePackageJson(version) {
  try {
    ...
    console.log(`📦 package.json: ${oldVersion} → ${version} ${isDryRun ? '(dry run)' : ''}`);
    return true;
  } catch (error) {
...
function updateAppJson(version) {
  try {
    ...
    console.log(`🎯 app.json: ${oldVersion} → ${version} ${isDryRun ? '(dry run)' : ''}`);
    return true;
  } catch (error) {
...
function updateIosProject(version, buildNumber) {
  try {
    ...
    if (oldMarketingVersion === version && oldBuildNumber >= buildNumber) {
      console.log(`✅ iOS project already at version ${version} (${buildNumber})`);
      return false;
    }
    ...
    console.log(`🍎 iOS: ${oldMarketingVersion} (${oldBuildNumber}) → ${version} (${buildNumber}) ${isDryRun ? '(dry run)' : ''}`);
    return true;
  } catch (error) {
...
function updateAndroidBuild(version, buildNumber) {
  try {
    ...
    console.log(`🤖 Android: ${oldVersionName} (${oldVersionCode}) → ${version} (${buildNumber}) ${isDryRun ? '(dry run)' : ''}`);
    return true;
  } catch (error) {
...
function main() {
  ...
  if (updatedCount === 0) {
    ...
    console.log(`🎉 Successfully ${isDryRun ? 'would update' : 'updated'} ${updatedCount} file(s) to version ${latestVersion}`);
