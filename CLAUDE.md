...
**iOS**: Requires Xcode 26.4+, iOS 16.4+ target. Device builds need a local signing team/profile, but the repo keeps `DEVELOPMENT_TEAM` empty for public portability. The last uploaded/processed TestFlight build is version `4.0.19` build `53` (`VALID` on `2026-06-01`, signed with Apple Distribution, archived/exported/uploaded via `asc`, `asc validate testflight --strict` clean, and distributed to the internal tester group); it carries the React Doctor `100/100` dead-code cleanup plus the reduce-motion and failed-send retry UX fixes. Internal App Store Connect IDs, tester group names, device names, and local artifact paths belong in private release notes, not public docs. `xcodebuild test` is not a gate yet because the `DNSChat` scheme has no XCTest bundles.
