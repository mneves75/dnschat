...
**iOS**: Requires Xcode 26.4+, iOS 16.4+ target. Device builds need a local signing team/profile, but the repo keeps `DEVELOPMENT_TEAM` empty for public portability. Current local release target is `4.1.5` build `72`: the Expo SDK 57 / React Native 0.86 upgrade with React kept at 19.2.3, refreshed iOS CocoaPods graph, and SDK 57 docs/release-state sync. Local evidence on `2026-06-30`: `bun run verify:all` passed, `cd modules/dns-native && bun run test` passed earlier in the lane, Xcode `26.6` (`17F113`) Debug simulator build passed, unsigned generic Release build passed, unsigned generic Release archive passed, and the compiled simulator app installed/launched with Argent after clearing a stale Metro process from another repo. TestFlight upload, App Store version attachment, and physical-device install remain separate claims and must not be inferred from local simulator/archive evidence. The latest uploaded TestFlight evidence remains `4.1.3` build `70` (`VALID` on `2026-06-22`) until a new `asc publish testflight` plus validation run proves otherwise. The Podfile clamps every pod target to `IPHONEOS_DEPLOYMENT_TARGET >= 16.4`; the Xcode 27 beta still cannot compile `expo-modules-jsi`, so use `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer` if the beta is globally selected. Runtime UI verification defaults to Argent MCP; in this SDK 57 lane the accessibility and React component-tree backends worked, while the screenshot/gesture simulator-server backend failed with `simulator-server exited with code before becoming ready`, so do not claim tap-flow proof from this run. Keep App Store Connect IDs, tester group names, device names, local paths, profile names, and signing identifiers out of public docs. `xcodebuild test` is not a native gate yet because the `DNSChat` scheme has no XCTest bundles.
