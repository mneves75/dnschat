...
**iOS**: Requires Xcode 26.4+, iOS 16.4+ target. Device builds need a local signing team/profile, but the repo keeps `DEVELOPMENT_TEAM` empty for public portability. The last uploaded/processed TestFlight build is version `4.0.18` build `51`; the current local release target is version `4.0.18` build `52` (post-review skeleton-width fix), and signed App Store archive/export, upload, and validation evidence must be collected before describing build `52` as distributed. Internal App Store Connect IDs, tester group names, device names, and local artifact paths belong in private release notes, not public docs. `xcodebuild test` is not a gate yet because the `DNSChat` scheme has no XCTest bundles.
