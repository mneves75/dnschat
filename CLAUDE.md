...
**iOS**: Requires Xcode 26.4+, iOS 16.4+ target. Device builds need a local signing team/profile, but the repo keeps `DEVELOPMENT_TEAM` empty for public portability. The last uploaded/processed TestFlight build is version `4.0.20` build `54` (`VALID` on `2026-06-02`, signed with Apple Distribution, archived/exported/uploaded via `asc`, `asc validate testflight --strict` clean, and distributed to the internal tester group). Build `54` hotfixes a launch crash in `4.0.19`/`53`: deriving `isReduceMotionEnabled` from an async `AccessibilityInfo` probe flipped it `false -> true` after mount and drove "Maximum update depth exceeded" for users with iOS Reduce Motion on. Reduce motion is now a stable in-app preference (`config.reduceMotion`); the fix was verified by launching the fixed build on a Simulator with system Reduce Motion enabled. Do not reintroduce an async OS-driven `shouldReduceMotion` flip without verifying animated screens on-device with Reduce Motion enabled. Internal App Store Connect IDs, tester group names, device names, and local artifact paths belong in private release notes, not public docs. `xcodebuild test` is not a gate yet because the `DNSChat` scheme has no XCTest bundles.
